...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --backup-from)
            ...
            BACKUP_FROM="$2"; shift 2 ;;
        -h|--help) usage; exit 0 ;;
        *) echo "ERROR: unknown option: $1"; usage; exit 2 ;;
    esac
done

for tool in git cmake ninja sha256sum ldd; do require_tool "$tool"; done
[[ -f "$PATCH" ]] || { echo "ERROR: missing patch: $PATCH"; exit 1; }
[[ -f "$TOOLTIP_PATCH" ]] || { echo "ERROR: missing tooltip patch: $TOOLTIP_PATCH"; exit 1; }

SYSTEM_PLUGIN="$(find_systemtray_plugin)" || { echo "ERROR: could not locate org.kde.plasma.systemtray.so"; exit 1; }
...
echo "Tested source commit: $BASE_COMMIT"
...
if [[ -n "$BACKUP_FROM" ]]; then
    if [[ -f "$BACKUP_FROM" && -r "$BACKUP_FROM" ]]; then
        ...
        BACKUP_FROM="$(sudo realpath "$BACKUP_FROM")"; BACKUP_FROM_NEEDS_SUDO=1
    else
        echo "ERROR: --backup-from file does not exist or cannot be accessed: $BACKUP_FROM"; exit 1
    fi
fi
...
if [[ ! -d "$SRC/.git" ]]; then git clone "$REPO_URL" "$SRC"; fi
...
[[ -f "$PLUGIN" ]] || { echo "ERROR: built plugin missing: $PLUGIN"; exit 1; }
if ldd "$PLUGIN" | grep -q 'not found'; then echo "ERROR: unresolved plugin dependencies:"; ldd "$PLUGIN" | grep 'not found'; exit 1; fi
...
[[ "$AFTER_SHA" == "$BUILT_SHA" ]] || { echo "ERROR: installed plugin hash does not match built plugin."; exit 1; }
